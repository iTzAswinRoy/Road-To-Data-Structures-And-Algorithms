...
public class SwappingNodes {     // Creating a class which handles linked list
    Node head;      // Creating a reference for head
    Node tail;      // Creating a reference for tail
    int size;      // Creating a size

    SwappingNodes(){     // Creating a constructor to initialize the size of the linked list
    ...
    public static class Node {   // Creating the Node class which handles data and its reference of the node
        ...
        Node(int value) {       // Creating a constructor to initialize the value of the node
    ...
    public void swapNode(int indexX, int indexY){      // Method to swap the two adjacent nodes
        Node prevX = null;      // Creating a 'previous' reference for index 1 which is pointing to null
        Node prevY = null;      // Creating a 'previous' reference for index 2 which is pointing to null

        Node currX = head;      // Creating a 'current' reference for index 1 which is pointing to head
        Node currY = head;      // Creating a 'current' reference for index 2 which is pointing to head
        ...
        if (indexX < 0 || indexX > size) {       // Checking if given index doesn't exceed the size of the linked list
            System.err.println("Index out of bound!");       // Displays an error message
        ...
        if (prevX != null) {    // Checking is the previous node of X contains a value
            ...
            head = currY;       // If the previous is still null, it means the index is at 0
        }
    ...
    public void display() {      // Method to display the linked list
        ...
        while (temp != null) {      // Creating a loop until it reaches the end of the linked list
            ...
            if (temp == null) {      // Initializing the temp variable to the reference of the tail
    ...
    public static void main(String[] args) {    // Main method
        SwappingNodes obj = new SwappingNodes();      // Creating an object ot call the methods
