...
// Write a function that subtracts one matrix from another of the same size.
public class FindingDifferenceInMatrix {
    public static void main(String[] args) {        // Main method
        FindingDifferenceInMatrix obj = new FindingDifferenceInMatrix();      // Creating an object to call the method
        ...
        try {
            // Calling the method to display difference in the matrix
            System.out.println("\nDifference of matrix:");
            obj.display(obj.subMatrix(mat1, mat2));         // Calling the method to find the difference in the matrix
    ...
    public int[][] subMatrix(int[][] data1, int[][] data2) {        // Method to find the difference in the matrix by passing 2 arguments
        ...
        for (int i = 0; i < data1.length; i++) {         // Iterating through the rows of the matrix
            ...
            for (int j = 0; j < data1[0].length; j++) {         // Iterating through the columns of the matrix
                result[i][j] = Math.abs(data1[i][j] - data2[i][j]);          // Storing the difference in values to the result matrix
            }
        }
